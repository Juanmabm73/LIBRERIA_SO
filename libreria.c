...
int tail(int N){
    ...
    while (fgets(buffer, sizeof(buffer), stdin) != NULL){
        ...
        if (count >= N) {      // despues de las N primeras lineas
            ...
            if (init == N){   // si llegamos al final del array con init el inicio pasa al principio
                init = 0;
            }
    ...
    if (count < N){
        ...
        for ( init; init < N; init++){ // de init al final 
        printf("%s", a[init]);
        }

        for ( i = 0; i < init; i++){   // desde el inicio hasta init
            printf("%s", a[i]);
        }
    }
...
int longlines(int N) {
    ...
    while (fgets(buffer, sizeof(buffer), stdin) != NULL) {
        ...
        if (i< N) { //array sin llenar 
            ...
            if (len > strlen(a[N])){    // vaciamos el ultimo hueco del array
                free(a[N]);
            }
